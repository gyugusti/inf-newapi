...
import { setFasilitas, changePage, getLkfKes, createLkfkes } from '@/redux-store/lkf/dataLkf'
...
import FormStorekes from './FormStorekes'
import { useSettings } from '@/@core/hooks/useSettings'

const PilihKesehatan = ({ data, open, handleClose }) => {
  ...
  const { kesehatanLkf, isLoading, numOfPages, total, current_page, per_page, search } = useSelector(
    store => store.dataLkf
  )
  ...
  const onSubmit = datacheck => {
    if (datacheck) {
      ...
      dispatch(createLkfkes(dataform))
  ...
  return (
    <Fragment>
      ...
      {/* Modal2 (FormTldLkf) */}
      <FormStorekes open={isModal2Open} handleClose={handleModal2Close} data={data} edit={false} />
    </Fragment>
  )
}

export default PilihKesehatan
