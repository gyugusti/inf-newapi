...
import { editLkfPekerja, editLkfSumber, lkfPekerjaStore } from '@/redux-store/lkf/dataLkf'
import { setTab } from '@/redux-store/lkf'

const FormPekerjaLkf = ({ data, open, edit, handleClose }) => {
  ...
  const onSubmit = dataform => {
    if (dataform) {
      if (edit) {
        dispatch(editLkfPekerja({ id: data.id, dataform }))
      } else {
        dispatch(lkfPekerjaStore({ lkf_id: data.lkf_id, ...dataform }))
      }
  ...
  return (
    <CustomDialog open={open} handleClose={handleClose} title='Edit Lkf Pekerja' maxWidth='sm'>
            ...
            <Controller
              ...
              render={({ field: { value, onChange } }) => (
                <CustomTextField
                  ...
                  helperText={errors.tgl_berlaku ? 'This field is required' : ''}
                />
              )}
            />
          </Grid>
