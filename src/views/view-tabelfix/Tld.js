...
import PilihTld from './modal/PilihTld'
import FormTldLkf from './modal/FormTldLkf'
import KonfirmasiDialog from '@/components/widget/KonfirmasiDialog'
import { deleteLkfDosis } from '@/redux-store/lkf/dataLkf'
import { setTab } from '@/redux-store/lkf'

const Tld = ({ data, detailLkf, action }) => {
  const dispatch = useDispatch()

  const tanggalLkf = '2022-07-25'
  ...
  return (
    <>
      ...
      {open && <PilihTld data={dataLkf} open={open} handleClose={handleModalClose} />}
      {edit && <FormTldLkf data={dataEdit} open={edit} edit={edit} handleClose={handleEditClose} />}
      ...
      <KonfirmasiDialog
        ...
        onConfirm={dataId => {
          if (dataId !== 'no') {
            dispatch(deleteLkfDosis({ id: dataId }))
