...
import { getLhiPekerja, setFasilitas, lhiDosisStore, lhiKesStore, editLhiKes } from '@/redux-store/lhi/dataLhi'
...
const FormStorekesLhi = ({ data, open, handleClose, edit, dataEdit }) => {
  const tahun = format(new Date(data.tgl_lhi), 'yyyy')

  const dispatch = useDispatch()
  const fas_id = data.alamat.fas_id

  const { pekerjaLhi, isLoading } = useSelector(store => store.dataLhi)
  ...
  const onSubmit = dataform => {
    if (dataform) {
      if (edit) {
        dispatch(editLhiKes({ id: dataEdit.id, dataform }))
      } else {
        dispatch(lhiKesStore({ lhi_id: data.lhi_id, ...dataform }))
      }
  ...
  useEffect(() => {
    if (debouncedSearchQuery && debouncedSearchQuery.length >= 3) {
      dispatch(getLhiPekerja(debouncedSearchQuery))
    }
  }, [dispatch, debouncedSearchQuery])

  useEffect(() => {
    const newOptions = pekerjaLhi.map(pekerja => ({
...
export default FormStorekesLhi
