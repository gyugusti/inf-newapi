...
import { getAlamat, getBidang, getJenisPekerja } from '@/redux-store/referensi-balis'
import Loading from '@/components/Loading'
import { editLhiPekerja, editLhiSumber, lhiPekerjaStore } from '@/redux-store/lhi/dataLhi'
import { setTab } from '@/redux-store/lhi'

const FormPekerjaLhi = ({ data, open, edit, handleClose }) => {
  // console.log(open)
  ...
  const onSubmit = dataform => {
    if (dataform) {
      if (edit) {
        dispatch(editLhiPekerja({ id: data.id, dataform }))
      } else {
        dispatch(lhiPekerjaStore({ lhi_id: data.lhi_id, ...dataform }))
      }
  ...
  return (
    <CustomDialog open={open} handleClose={handleClose} title='Edit Lhi Pekerja' maxWidth='sm'>
        ...
        <Grid container spacing={3}>
          ...
          <Grid size={{ xs: 12, sm: 6 }}>
            <Controller
              ...
              control={control}
              render={({ field: { value, onChange } }) => (
                <CustomTextField fullWidth value={value} label='No SIB' onChange={onChange} />
              )}
            />
          </Grid>
          ...
          <Grid size={{ xs: 12, sm: 6 }}>
            <Controller
              ...
              control={control}
              render={({ field: { value, onChange } }) => (
                <CustomTextField fullWidth value={value} label='Tlp / No HP' onChange={onChange} />
              )}
            />
          </Grid>
