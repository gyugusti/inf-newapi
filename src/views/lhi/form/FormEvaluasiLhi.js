...
const FormEvaluasiLhi = ({ inkf, temuanItem, detailIkk }) => {
  // Initialize state with temuanItem data

  const [temuanValues, setTemuanValues] = useState(
    temuanItem.reduce((acc, item) => {
      acc[item.temuan_id] = {
        hasilEva: item.lhi_temuan?.hasil_temuan_id?.toString() || '2', // Default value for radio
        catatan: item.lhi_temuan?.uraian || '' // Default text for CustomTextField
      }
...
export default FormEvaluasiLhi
