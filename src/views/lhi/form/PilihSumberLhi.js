...
import { getLhiSumber, changePageSrp, handleChangeSrp, createLhiSumber, setFasilitas } from '@/redux-store/lhi/dataLhi'
import { useSettings } from '@/@core/hooks/useSettings'

const PilihSumberLhi = ({ data, open, handleClose }) => {
  ...
  const { sumberLhi, isLoadingSrp, numOfPagesSrp, totalSrp, current_pageSrp, per_page, searchSrp } = useSelector(
    store => store.dataLhi
  )
  ...
  const onSubmit = datacheck => {
    if (datacheck) {
      ...
      if (edit) {
        ...
        dispatch(createLhiSumber(dataform))
      }
  ...
  return (
    <CustomDialog open={open} handleClose={handleClose} title='Tambah Data Sumber FIHI' maxWidth='lg'>
      {/* Dialog content */}
      {isLoadingSrp ? (
        <Loading />
      ) : (
