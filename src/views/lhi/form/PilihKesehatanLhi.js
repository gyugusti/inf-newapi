...
import { setFasilitas, changePage, getLhiKes, createLhikes } from '@/redux-store/lhi/dataLhi'
import { setTab } from '@/redux-store/lhi'
import Loading from '@/components/Loading'
import SearchSumber from '@/views/lvkf/frontend/SearchSumber'

import FormStorekesLhi from './FormStorekesLhi'
import { useSettings } from '@/@core/hooks/useSettings'

const PilihKesehatanLhi = ({ data, open, handleClose }) => {
  ...
  const { kesehatanLhi, isLoading, numOfPages, total, current_page, per_page, search } = useSelector(
    store => store.dataLhi
  )
  ...
  const onSubmit = datacheck => {
    if (datacheck) {
      ...
      dispatch(createLhikes(dataform))
  ...
  return (
    <Fragment>
      ...
      {/* Modal2 (FormTldLhi) */}
      <FormStorekesLhi open={isModal2Open} handleClose={handleModal2Close} data={data} edit={false} />
    </Fragment>
  )
}

export default PilihKesehatanLhi
