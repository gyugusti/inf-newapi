...
import { setFasilitas, getLhiPekerja, changePage, createLhiPekerja } from '@/redux-store/lhi/dataLhi'
...
import FormPekerjaLhi from './FormPekerjaLhi'
import { useSettings } from '@/@core/hooks/useSettings'

const PilihPekerjaLhi = ({ data, open, handleClose }) => {
  ...
  const { pekerjaLhi, isLoading, numOfPages, total, current_page, per_page, search } = useSelector(
    store => store.dataLhi
  )
  ...
  const onSubmit = datacheck => {
    if (datacheck) {
      ...
      dispatch(createLhiPekerja(dataform))
  ...
  return (
    <Fragment>
      ...
      <FormPekerjaLhi open={isModal2Open} handleClose={handleModal2Close} data={data} edit={false} />
    </Fragment>
  )
}

export default PilihPekerjaLhi
