...
import PilihKesehatanLhi from '../form/PilihKesehatanLhi'
import KonfirmasiDialog from '@/components/widget/KonfirmasiDialog'
import { setTab } from '@/redux-store/lhi'
import { deleteLhiKes } from '@/redux-store/lhi/dataLhi'
import FormStorekesLhi from '../form/FormStorekesLhi'

const Kesehatan = ({ data, detailLhi, action }) => {
  ...
  return (
    <>
      ...
      {open && <PilihKesehatanLhi data={dataLhi} open={open} handleClose={handleModalClose} />}

      {edit && (
        <FormStorekesLhi data={dataLhi} dataEdit={dataEdit} open={edit} edit={edit} handleClose={handleEditClose} />
      )}
      ...
      <KonfirmasiDialog
        ...
        onConfirm={dataId => {
          if (dataId !== 'no') {
            dispatch(deleteLhiKes({ id: dataId }))
