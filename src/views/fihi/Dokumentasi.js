...
import { deleteFihiDok, getDokumenfihi, setNamadoc, setTab } from '@/redux-store/fihi'
import { getFihiDok } from '@/redux-store/fihi/dataFihi'

const Dokumentasi = ({ dataFihi }) => {
  ...
  const { dataFihidok } = useSelector(store => store.fihi)

  const handleClickDownload = (id, nama) => {
    dispatch(setNamadoc(nama))
    dispatch(getDokumenfihi(id))
  }
  ...
  return (
    <>
      ...
      <Grid>
        <Box>
          {dataFihi.dokumen &&
            dataFihi.dokumen.map((item, index) => {
              return (
                <div key={index}>
      ...
      <KonfirmasiDialog
        ...
        onConfirm={dataId => {
          if (dataId !== 'no') {
            dispatch(deleteFihiDok({ id: dataId }))
