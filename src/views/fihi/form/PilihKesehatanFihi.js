...
import { setFasilitas, changePage, getFihiKes, createFihikes } from '@/redux-store/fihi/dataFihi'
import { setTab } from '@/redux-store/fihi'
import Loading from '@/components/Loading'
import SearchSumber from '@/views/lvkf/frontend/SearchSumber'

import FormStorekesFihi from './FormStorekesFihi'
import { useSettings } from '@/@core/hooks/useSettings'

const PilihKesehatanFihi = ({ data, open, handleClose }) => {
  ...
  const { kesehatanFihi, isLoading, numOfPages, total, current_page, per_page, search } = useSelector(
    store => store.dataFihi
  )
  ...
  const onSubmit = datacheck => {
    if (datacheck) {
      ...
      const dataform = {
        fihi_id: data.fihi_id,
        pekerja: datacheck.pilih
      }

      dispatch(createFihikes(dataform))
  ...
  return (
    <Fragment>
          ...
          <form id='myForm' onSubmit={handleSubmit(onSubmit)}>
            ...
            <TableContainer>
              <Table sx={{ minWidth: 650 }} aria-label='simple table'>
                <TableHead>
                  <TableRow sx={{ backgroundColor: bgColors.primary }}>
      ...
      {/* Modal2 (FormTldFihi) */}
      <FormStorekesFihi open={isModal2Open} handleClose={handleModal2Close} data={data} edit={false} />
    </Fragment>
  )
}

export default PilihKesehatanFihi
