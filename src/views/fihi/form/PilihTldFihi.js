...
import { setFasilitas, getFihiDosis, changePage, createFihiDosis } from '@/redux-store/fihi/dataFihi'
import { setTab } from '@/redux-store/fihi'
import Loading from '@/components/Loading'
import SearchSumber from '@/views/lvkf/frontend/SearchSumber'
import FormStoreTldFihi from './FormStoreTldFihi'
import { useSettings } from '@/@core/hooks/useSettings'

const PilihTldFihi = ({ data, open, handleClose }) => {
  ...
  const { dosisFihi, isLoading, numOfPages, total, current_page, per_page, search } = useSelector(
  ...
  const onSubmit = datacheck => {
    if (datacheck) {
      ...
      dispatch(createFihiDosis(dataform))
  ...
  return (
    <Fragment>
      ...
      <FormStoreTldFihi open={isModal2Open} handleClose={handleModal2Close} data={data} edit={false} />
    </Fragment>
  )
}

export default PilihTldFihi
