...
import { setFasilitas, getFihiPekerja, changePage, createFihiPekerja } from '@/redux-store/fihi/dataFihi'
...
import FormPekerjaFihi from './FormPekerjaFihi'
import { useSettings } from '@/@core/hooks/useSettings'

const PilihPekerjaFihi = ({ data, open, handleClose }) => {
  ...
  const { pekerjaFihi, isLoading, numOfPages, total, current_page, per_page, search } = useSelector(
  ...
  const onSubmit = datacheck => {
    if (datacheck) {
      ...
      dispatch(createFihiPekerja(dataform))
  ...
  return (
    <Fragment>
      ...
      <FormPekerjaFihi open={isModal2Open} handleClose={handleModal2Close} data={data} edit={false} />
    </Fragment>
  )
}

export default PilihPekerjaFihi
