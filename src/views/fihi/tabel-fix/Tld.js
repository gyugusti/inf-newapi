...
import PilihTldFihi from '../form/PilihTldFihi'
import FormTldFihi from '../form/FormTldFihi'
import KonfirmasiDialog from '@/components/widget/KonfirmasiDialog'
import { deleteFihiDosis } from '@/redux-store/fihi/dataFihi'
import { setTab } from '@/redux-store/fihi'

const Tld = ({ data, detailFihi, action }) => {
  ...
  return (
    <>
      ...
      {open && <PilihTldFihi data={dataFihi} open={open} handleClose={handleModalClose} />}

      {edit && <FormTldFihi data={dataEdit} open={edit} edit={edit} handleClose={handleEditClose} />}
      ...
      <KonfirmasiDialog
        ...
        onConfirm={dataId => {
          if (dataId !== 'no') {
            dispatch(deleteFihiDosis({ id: dataId }))
