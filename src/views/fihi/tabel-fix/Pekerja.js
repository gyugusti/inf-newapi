...
import PilihPekerjaFihi from '../form/PilihPekerjaFihi'
import FormPekerjaFihi from '../form/FormPekerjaFihi'
import KonfirmasiDialog from '@/components/widget/KonfirmasiDialog'
import { deleteFihiPekerja } from '@/redux-store/fihi/dataFihi'
import { setTab } from '@/redux-store/fihi'

const Pekerja = ({ data, detailFihi, action }) => {
  ...
  return (
    <>
      ...
      {open && <PilihPekerjaFihi data={dataFihi} open={open} handleClose={handleModalClose} />}
      {edit && <FormPekerjaFihi data={dataEdit} open={edit} edit={edit} handleClose={handleEditClose} />}
      ...
      <KonfirmasiDialog
        ...
        onConfirm={dataId => {
          if (dataId !== 'no') {
            dispatch(deleteFihiPekerja({ id: dataId }))
