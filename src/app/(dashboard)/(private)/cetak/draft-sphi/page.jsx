...
import { getDraftSphi } from '@/redux-store/pdf'
...
const LhiCetak = () => {
  ...
  const { draftSphi } = useSelector(store => store.pdf)

  // Dispatching the action to get PDF data when Id is available
  useEffect(() => {
    if (Id !== null) {
      dispatch(getDraftSphi(Id))
    }
  }, [dispatch, Id])

  // Constructing the PDF source (base64) when dataSbi is updated
  const pdfSrc = draftSphi ? `data:application/pdf;base64,${draftSphi}` : null
