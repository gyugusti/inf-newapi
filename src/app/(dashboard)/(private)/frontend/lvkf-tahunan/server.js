...
export async function getLkfData(params = {}) {
  ...
  // Pastikan user ada (opsional, karena route juga cek; ini buat fail fast)
  ...
  // Buat absolute base URL dari header (works on Vercel & dev)
  const h = nextHeaders()
  ...
  try {
    ...
    const res = await fetch(`${base}/api/lvkf-tahunan?${qp.toString()}`, {
      method: 'GET',
      headers: {
        // Forward cookies agar getServerSession di API Route terbaca
        cookie: cookieHeader
      },
      cache: 'no-store' // dinamis, setara behavior sebelumnya
    })
